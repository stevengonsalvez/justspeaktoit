...
| Provider selection and fallback | `SettingsScreen` exposes Android Speech, Deepgram, ElevenLabs, OpenAI; `SpeakViewModel.resolvedModel` falls back to Android Speech when keys are missing | `ParityLogicTest.parityChecklist_coversMajorGoalSurfaces`; Gradle unit tests |
| Secure API keys | `SpeakRepository` stores keys with `EncryptedSharedPreferences` and falls back to private prefs if device crypto setup is unavailable | `MainActivityParityTest.settingsFlow_exposesApiPostProcessingAndOpenClawConfiguration` |
| Transcript history | `HistoryScreen`, `HistoryEntry`, and JSON persistence in `SpeakRepository` | `MainActivityParityTest.transcribeFlow_recordsCopiesAndShowsHistory` |
| Post-processing/polish | `PostProcessingScreen` and `TranscriptFormatter.polish`; model and prompt settings retained | `ParityLogicTest.transcriptFormatter_polishesSpacingCasingAndPunctuation` |
| OpenClaw conversations | `OpenClawListScreen`, `OpenClawChatScreen`, `Conversation`, and persisted messages | `MainActivityParityTest.openClawFlow_createsConversationAndReceivesAssistantResponse` |
| OpenClaw gateway protocol helpers | `OpenClawProtocol.normaliseGatewayUrl`, keyword acknowledgement, and cleanup helpers | `ParityLogicTest.normaliseGatewayUrl_*`, `ParityLogicTest.keywordAcknowledge_*` |
...
| TTS controls | `OpenClawSettingsScreen` exposes speak responses, summarise responses, low-latency speech, voice/model/speed state | Gradle build and Compose settings coverage |
